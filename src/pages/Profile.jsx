...
import FavouritePage from './FavouritePage';
 // Make sure 'Header' matches the filename
...
export default function Profile() {
  ...
  const fetchMediaItems = async () => {
    try {
      ...
      const Data = data.filter(item => String(item.id) === String(decoded.id));
  ...
  const fetchUserInfo = async () => {
    try {
      ...
      const data = await response.json();
      setUserInfo(data); // <- Now userInfo will be a single object
    } catch (error) {
        ...
        <div className="flex-1 p-6 overflow-auto">
          ...
          <div className="flex border-b border-gray-200 mb-6">
            ...
            <button 
              ...
              className={`px-4 py-2 font-medium ${activeTab === 'favourites' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}
            >
              Favourites
            </button>
          </div>
